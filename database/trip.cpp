...
namespace database
{
    ...
    std::optional<Trip> Trip::read_by_id(long id)
    {
        try
        {
            ...
            Trip a;
            select << "SELECT id, route_id, user_id, trip FROM Trip where id=?",
                into(a._id),
                into(a._route_id),
                into(a._user_id),
                into(a._trip),
    ...
    void Trip::save_to_mysql()
    {
        ...
        try
        {
            ...
            insert << "INSERT INTO Trip (route_id, user_id, trip) VALUES(?, ?, ?)",
                use(_route_id),
                use(_user_id),
                use(_trip);
    ...
    std::vector<Trip> trip::read_by_route_id(long route_id)
    {
        try
        {
            ...
            select << "SELECT id, route_id, user_id, trip FROM Trip WHERE Trip.route_id=?",
                into(a.id()),
                into(a.route_id()),
                into(a.user_id()),
                into(a.trip()),
