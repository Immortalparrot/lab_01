...
namespace database
{
    ...
    std::vector<Route> RouteToTrip::read_routes_by_user_id(long user_id)
    {
        try
        {
            ...
            Route a;
            select << "SELECT DISTINCT Route.id, Route.name FROM Route INNER JOIN RouteToTrip ON Route.id = RouteToTrip.route_id WHERE RouteToTrip.user_id=?",
                into(a.id()),
                into(a.name()),
                use(user_id),
    ...
    void RouteToTrip::save_to_mysql()
    {
        ...
        try
        {
            ...
            insert << "INSERT INTO RouteToTrip (route_id, user_id) VALUES(?, ?)",
                use(_route_id),
                use(_user_id);
